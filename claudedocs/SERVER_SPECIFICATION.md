...
**지원 파일 타입:**
- `image`: 모든 image/* MIME 타입 (JPEG, PNG, WebP, HEIC, HEIF 등)
- `video`: 모든 video/* MIME 타입 (MP4, WebM, MOV, M4V 등) - 자동 처리

**iPhone 호환성:**
- HEIC/HEIF 이미지: 변환 없이 직접 업로드 가능
- MOV/M4V 동영상: 변환 없이 직접 업로드 가능
- 자동으로 모든 Apple 기기 포맷 지원

**MIME 타입 검증:**
- 이미지 업로드: `image/*` 접두사 검증 (접두사 기반)
- 비디오 업로드: `video/*` 접두사 검증 (접두사 기반)
- 유연한 검증: 새로운 이미지/비디오 포맷 자동 지원
- 보안: PDF, 텍스트, JSON 등 유효하지 않은 타입 거부
...
**권한 타입:**
- `read`: 읽기 전용 (파일 보기 및 다운로드)
- `write`: 읽기 + 쓰기 (파일 보기, 다운로드, 업로드, 수정)

**공유 폴더 파일 접근:**
- 공유된 폴더의 파일 목록 조회: `GET /api/v1/folders/:id/files`
  - 폴더 소유자와 공유 받은 사용자 모두 접근 가능
  - 권한 검증: 소유자 확인 또는 `folder_shares` 테이블 조회
  - 반환 파일: 폴더 소유자의 파일 전체 (공유 받은 사용자 ID로 필터링하지 않음)
  - 보안: 권한 검증 후에만 파일 데이터 조회
  - 삭제된 공유 기록(`deleted_at IS NOT NULL`)은 접근 불가
...
└─────────────────┬───────────────────────────────┘
                  ...
                  ▼
┌─────────────────────────────────────────────────┐
│ 2. HasFolderAccess 권한 검증 (인가)              │
│    ├─ 폴더 소유자 확인                           │
│    │  (folders.user_id = 요청자 ID)             │
│    └─ 공유 받은 사용자 확인                      │
│       (folder_shares WHERE shared_with_id = ?)  │
└─────────────────┬───────────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────────┐
...
│ 4. 파일 목록 조회                                │
│    (폴더 소유자 ID로 필터링)                     │
│    WHERE folder_id = ? AND user_id = 소유자 ID  │
└─────────────────────────────────────────────────┘
```

**동작 예시:**
```
상황: User A(ID:1)가 Folder X를 User B(ID:2)와 공유 (read 권한)

User B가 GET /api/v1/folders/X/files 요청 시:
1. JWT 토큰에서 User B(ID:2) 확인
2. HasFolderAccess(userID=2, folderID=X) 호출
   - 소유자 확인: folders WHERE id=X AND user_id=2 → 실패 (소유자는 1)
   - 공유 확인: folder_shares WHERE folder_id=X AND shared_with_id=2 → 성공 (read 권한)
3. 접근 허용 → 폴더 정보 조회 (소유자 ID=1 확인)
4. 파일 조회: cloud_files WHERE folder_id=X AND user_id=1
5. 결과: User A의 파일 목록 반환 (User B가 볼 수 있음)
```
