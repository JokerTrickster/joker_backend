...
# 1. 구글 계정으로 처음 로그인
# 2. 새 사용자 레코드 생성 확인
# 3. Access Token과 Refresh Token 수신 확인
```

### 시나리오 2: 기존 사용자 로그인
```bash
# 1. 이미 등록된 구글 계정으로 로그인
# 2. 기존 사용자 정보 조회 확인
# 3. 새로운 JWT 토큰 발급 확인
```
...
### 시나리오 3: 삭제된 계정 복원
```bash
# 1. 이전에 삭제(Soft-Delete)된 계정으로 로그인 시도
# 2. 계정 자동 복원 확인 (deleted_at → NULL)
# 3. 기존 사용자 데이터 유지 확인 (user ID, created_at 등)
# 4. 정상적으로 JWT 토큰 발급 확인
```

**삭제된 계정 복원 동작:**
- 삭제된 계정으로 구글 로그인 시 자동 복원
- "Failed to find or create user" 에러 발생하지 않음
- 기존 사용자 데이터 완전히 보존
- 새 계정 생성되지 않음
- 사용자에게 투명하게 처리됨
