...
type ListCloudRepositoryHandler struct {
	UseCase _interface.IListCloudRepositoryUseCase
}

func NewListCloudRepositoryHandler(c *echo.Group, useCase _interface.IListCloudRepositoryUseCase) _interface.IListCloudRepositoryHandler {
	handler := &ListCloudRepositoryHandler{
		UseCase: useCase,
	}
	c.GET("/files", handler.ListFiles)
	return handler
}
...
// @Success 200 {object} response.ListFilesResponseDTO
// @Failure 400 {object} map[string]string
// @Failure 500 {object} map[string]string
// @Router /api/v1/files [get]
func (h *ListCloudRepositoryHandler) ListFiles(c echo.Context) error {
	...
	var req request.ListFilesRequestDTO
